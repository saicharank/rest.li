 ...
 */
public class TestStreamEcho extends AbstractServiceTest
{
  ...
  public TestStreamEcho(ClientProvider clientProvider, ServerProvider serverProvider, int port) {
  ...
  private void testNormalEcho(long bytesNum, URI uri) throws Exception
  {
    ...
    StreamRequest request =
        new StreamRequestBuilder(_clientProvider.createHttpURI(_port, uri)).build(EntityStreams.newEntityStream(writer));
  ...
  public void testDelayedEcho() throws Exception
  {
    RestRequest restRequest =
        new RestRequestBuilder(_clientProvider.createHttpURI(_port, DELAYED_ECHO_URI)).setEntity("wei ni hao ma?".getBytes())
            .build();
